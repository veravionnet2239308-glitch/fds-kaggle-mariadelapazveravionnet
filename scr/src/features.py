...
def extract_features(data):
    """
    Given a list of battles (JSONL objects), returns a DataFrame
    with all engineered features used in the final model.
    """
    rows = []
    for b in tqdm(data, desc="Extracting features"):
        ...
        # --- Base stats: means, lead and deltas ---
        ...
        # --- Offensive / defensive composite metrics ---
        ...
        for t in tl:
            ...
            # First strike estimation (priority + speed)
            if p1m and p2m:
                ...
                if p1_pri > p2_pri:
                    ...
                    if r["p1_mean_base_spe"] > r["p2lead_base_spe"]:
                        ...
                        p2_first_strike += 1
            elif p1m and not p2m:
                p1_first_strike += 1
            elif p2m and not p1m:
                p2_first_strike += 1
        ...
        total_dmg_p1 = sum(damage_turn_p1)
        total_dmg_p2 = sum(damage_turn_p2)
        r["damage_to_p1"] = total_dmg_p1
        r["damage_to_p2"] = total_dmg_p2
        r["damage_diff"]  = total_dmg_p2 - total_dmg_p1
        ...
        r["max_damage_p1"]   = float(np.max(damage_turn_p1)) if damage_turn_p1 else 0.0
        r["max_damage_p2"]   = float(np.max(damage_turn_p2)) if damage_turn_p2 else 0.0
        r["max_damage_diff"] = r["max_damage_p2"] - r["max_damage_p1"]
